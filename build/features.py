...
class LiveBroadcasting(Feature):
    ...
    def configure(self, build, conf):
        ...
        if build.platform_is_linux and not int(build.flags['shoutcast_internal']):
            ...
            if not conf.CheckForPKG('shout-idjc', '2.4.3'):
                if conf.CheckForPKG('shout-idjc', '2.4.2'):
        ...
        if not libshout_found:
            raise Exception('Could not find libshout-idjc or its development headers. Please install it or compile Mixxx without Shoutcast support using the shoutcast=0 flag.')
