...
DeckVisuals::DeckVisuals(const QString& group)
        : m_group(group),
          m_SlowTickCnt(0),
          m_trackLoaded(false),
          m_playButton(ConfigKey(group, "play")),
          m_loopEnabled(ConfigKey(group, "loop_enabled")),
          m_engineBpm(ConfigKey(group, "bpm")),
          m_engineKey(ConfigKey(group, "key")) {
...
void DeckVisuals::process(double remainingTimeTriggerSeconds) {
    ...
    if (m_SlowTickCnt == 0 || !trackLoaded) {
        m_pVisualBpm->set(m_engineBpm.get());
    }
    m_pVisualKey->set(m_engineKey.get());
