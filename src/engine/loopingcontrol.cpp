...
double LoopingControl::s_dBeatSizes[] = {0.03125,
        0.0625,
        0.125,
        0.25,
        0.5,
...
LoopingControl::LoopingControl(QString group,
    ...
    connect(m_pReloopAndStopButton,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotReloopAndStop(double)),
            Qt::DirectConnection);
    ...
    m_pCOBeatLoopSize->connectValueChangeRequest(this,
            SLOT(slotBeatLoopSizeChangeRequest(double)),
            Qt::DirectConnection);
    m_pCOBeatLoopActivate = new ControlPushButton(ConfigKey(group, "beatloop_activate"));
    connect(m_pCOBeatLoopActivate,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotBeatLoopToggle(double)));
    m_pCOBeatLoopRollActivate = new ControlPushButton(ConfigKey(group, "beatlooproll_activate"));
    connect(m_pCOBeatLoopRollActivate,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotBeatLoopRollActivate(double)));
    ...
    for (unsigned int i = 0; i < (sizeof(s_dBeatSizes) / sizeof(s_dBeatSizes[0])); ++i) {
        ...
        connect(pBeatLoop,
                SIGNAL(activateBeatLoop(BeatLoopingControl*)),
                this,
                SLOT(slotBeatLoopActivate(BeatLoopingControl*)),
                Qt::DirectConnection);
        connect(pBeatLoop,
                SIGNAL(activateBeatLoopRoll(BeatLoopingControl*)),
                this,
                SLOT(slotBeatLoopActivateRoll(BeatLoopingControl*)),
                Qt::DirectConnection);
        connect(pBeatLoop,
                SIGNAL(deactivateBeatLoop(BeatLoopingControl*)),
                this,
                SLOT(slotBeatLoopDeactivate(BeatLoopingControl*)),
                Qt::DirectConnection);
        connect(pBeatLoop,
                SIGNAL(deactivateBeatLoopRoll(BeatLoopingControl*)),
                this,
                SLOT(slotBeatLoopDeactivateRoll(BeatLoopingControl*)),
                Qt::DirectConnection);
        m_beatLoops.append(pBeatLoop);
    }

    m_pCOBeatJump = new ControlObject(ConfigKey(group, "beatjump"), false);
    connect(m_pCOBeatJump,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotBeatJump(double)),
            Qt::DirectConnection);
    ...
    connect(m_pCOBeatJumpForward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotBeatJumpForward(double)));
    m_pCOBeatJumpBackward = new ControlPushButton(ConfigKey(group, "beatjump_backward"));
    connect(m_pCOBeatJumpBackward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotBeatJumpBackward(double)));
    ...
    connect(m_pCOLoopMove,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotLoopMove(double)),
            Qt::DirectConnection);
    ...
    connect(m_pCOLoopScale, SIGNAL(valueChanged(double)), this, SLOT(slotLoopScale(double)));
    m_pLoopHalveButton = new ControlPushButton(ConfigKey(group, "loop_halve"));
    connect(m_pLoopHalveButton, SIGNAL(valueChanged(double)), this, SLOT(slotLoopHalve(double)));
    m_pLoopDoubleButton = new ControlPushButton(ConfigKey(group, "loop_double"));
    connect(m_pLoopDoubleButton, SIGNAL(valueChanged(double)), this, SLOT(slotLoopDouble(double)));
...
void LoopingControl::setLoopInToCurrentPosition() {
    ...
    if (m_pQuantizeEnabled->toBool() && loopSamples.start < loopSamples.end && pBeats) {
...
void LoopingControl::trackLoaded(TrackPointer pNewTrack) {
    if (m_pTrack) {
        disconnect(m_pTrack.get(), SIGNAL(beatsUpdated()), this, SLOT(slotUpdatedTrackBeats()));
    }
    ...
    if (pNewTrack) {
        ...
        connect(m_pTrack.get(), SIGNAL(beatsUpdated()), this, SLOT(slotUpdatedTrackBeats()));
    } else {
        m_pTrack.reset();
        m_pBeats.clear();
    }
...
void LoopingControl::slotBeatLoop(double beats, bool keepStartPoint, bool enable) {
    ...
    if (newloopSamples.start >=
        ...
        if (previousBeatloopOutPoint < newloopSamples.start && beats < previousBeatloopSize) {
            m_pCOBeatLoopSize->setAndConfirm(beats);
        }
        return;
    }
    ...
    if (BpmControl::getBeatContext(pBeats,
        ...
        double new_loop_out = currentLoopMatchesBeatloopSize()
                ? pBeats->findNBeatsFromSample(
                          new_loop_in, m_pCOBeatLoopSize->get())
...
double LoopingControl::seekInsideAdjustedLoop(
        double currentSample, double old_loop_in, double new_loop_in, double new_loop_out) {
...
BeatJumpControl::BeatJumpControl(QString group, double size)
    ...
    connect(m_pJumpForward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotJumpForward(double)),
            Qt::DirectConnection);
    m_pJumpBackward = new ControlPushButton(
            keyForControl(group, "beatjump_%1_backward", size));
    connect(m_pJumpBackward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotJumpBackward(double)),
            Qt::DirectConnection);
}
...
LoopMoveControl::LoopMoveControl(QString group, double size)
    ...
    connect(m_pMoveForward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotMoveForward(double)),
            Qt::DirectConnection);
    m_pMoveBackward = new ControlPushButton(
            keyForControl(group, "loop_move_%1_backward", size));
    connect(m_pMoveBackward,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotMoveBackward(double)),
            Qt::DirectConnection);
}
...
BeatLoopingControl::BeatLoopingControl(QString group, double size)
    ...
    connect(m_pActivateRoll,
            SIGNAL(valueChanged(double)),
            this,
            SLOT(slotActivateRoll(double)),
            Qt::DirectConnection);
