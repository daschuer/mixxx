...
BUILDENV_NAME="mixxx-deps-2.4-arm64-osx-min11.0-0275662"
    ...
    setup)
        ...
        if [ ! -d "${BUILDENV_PATH}" ]; then
            if [ "$1" != "--profile" ]; then
                ...
                if curl -o "${BUILDENV_PATH}_.zip" -L -H "authorization: token $2" -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/daschuer/vcpkg/actions/artifacts/141919083/zip; then
